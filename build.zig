...
pub fn build(b: *std.Build) void {
    ...
    // The default stack overflowed at boot when a group grew from 24 bodies to `worldfmt.MAX_FOES`. Windows COMMITS stack lazily, so this is address space and not memory. "WHAT THE FRAME COSTS" measures 144.4 MB of slabs across 29 rows against this 192 MB.
    ...
    // DEV ONLY: `zig build test -Dtest-filter=knight` runs only the tests whose name contains that.
    ...
    // DEV ONLY: `zig build check` type-checks and stops. `Step.Compile` passes `-fno-emit-bin` whenever nothing asks for its binary, so no step here may install or run these two: 1.4 s against 8.4 s, because sema is 1.4 s of a build and LLVM plus LLD are the other 7.
...
/// THE TEST ROSTER IS A LOCKSTEP LIST, AND NOTHING WAS CHECKING IT: a module missing from `main.zig`'s `test { _ = @import(…) }` block reports "all tests passed" while every test it carries goes unrun. A comptime assert cannot see the filesystem, so the check lives HERE — every `src/**/*.zig` named by the path `main.zig` imports it as.
/// AND IT IS SCOPED TO THE BLOCK, NOT TO THE FILE: searching the whole of `main.zig` counted the ORDINARY imports at the top of it, and a top-level import pulls no tests in.

/// Bytes under which a `src/**/*.zig` is a truncation rather than a module — the smallest real one is `foes/foestat.zig` at 2468.
const MIN_SRC: u64 = 512;

/// EVERY COMPILE GETS THE SAME ASSETS: the exe, the test root and the two `check` compiles each named the embeds themselves, and one added to the exe and not to `check` makes `check.cmd` pass on a build that cannot link.
...
fn checkTestRoster(b: *std.Build) void {
        ...
        std.debug.panic("src/ walk failed part-way ({s}) — the roster check saw only some of the tree", .{@errorName(e)})) |ent|
    {
        ...
        // AND A MODULE NAMED BUT EMPTIED IS THE SAME FAILURE WITH NOTHING LEFT TO RUN: `ui/editor.zig` was truncated to 0 bytes and committed that way (c08f4f7), and this walk passed. Anything under `MIN_SRC` is a truncation.
