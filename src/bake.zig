...
// ── THE BAKE ── a ONE-WAY door, run once by `--bake`: the world that used to be authored in
// env.zig's five region functions, emitted as the op list of `worlds/01_fallen_plain.world`.
// After this the map file is the world and this module is history — kept in the tree because
// it is the readable record of where the first map came from, and because re-baking is the
// only way back if the format ever needs a rebuild from source.
//
// It is a PORT, not a transcription: `Emit` carries the same method names and signatures the
// old `Placer` did (at / atY / jit / belt / ring / anyOf), so the region paragraphs below are
// the originals with their bodies untouched, and anything that no longer typechecks is a
// difference the compiler makes me look at rather than one I get to overlook.
//
// WHAT CHANGES, deliberately: a call that scattered N instances now emits ONE op describing
// the scatter, and each op gets its OWN seed. The composition is preserved exactly — same
// boxes, same counts, same scale bands, same mixes, same rejections — but the individual
// jitter lands differently, because independent streams are what make an edit local. See
// worldfmt.zig's header.
//
// WHAT DOESN'T: a call that placed ONE prop still places one prop, at the same coordinates.
// Where the position was computed from the shared rng (`h[2] + rng.signed()*8`), the bake
// resolves it here and writes the literal, so those hand compositions survive to the metre.
...
pub const Emit = struct {
    ...
    // Per-op seeds, handed out SEQUENTIALLY. Consecutive integers are fine — Zig's DefaultPrng
    // runs its seed through SplitMix64 to fill the state, so 1001 and 1002 give uncorrelated
    // streams — and they keep the file legible: a hashed 20-digit seed is noise you can't read
    // in a diff or retype after a re-roll. Counter-derived rather than drawn from `rng`, so
    // inserting a literal `at` between two belts doesn't renumber every seed after it.
    ...
    // ── the old Placer's surface, emitting ops ──────────────────────────────────────

    fn at(self: *Emit, kind: Kind, x: f32, z: f32, yaw: f32, scale: f32) void {
        self.atY(kind, x, 0, z, yaw, scale);
    }

    // Y is carried on the op only for water, which staggers overlapping sheets out of
    // z-fighting; everything else is authored on the ground.
    ...
    // A jittered single stamp RESOLVES here: it was one prop then and it is one prop now, so
    // it becomes a literal rather than a one-instance generator nobody would ever want to dial.
    ...
    // A belt of MIXED kinds, optionally thinned along an axis — the wood's canopy, which the
    // old code wrote as a bespoke loop because the Placer had no way to say either thing.
    ...
    // An annulus scatter about a centre: the shoreline forms the old code spelled out as
    // `angle + range(r0, r1)` loops, four times over, once per bed.
...
// the same convention the colliders use. Unchanged from env.zig; the door and torch positions
// it computes are baked as literals.
...
pub fn build(m: *wf.Map) void {
    ...
    // The clearings the canopy and the ground cover keep out of. In env.zig these were the
    // hard-coded `nearClearing`; as data they are draggable, and the scatter reads them all.
...
// The three foe groups' old hard-coded `homes` tables, moved into the map where the editor can
// reach them. Yaw in DEGREES here (the rigs take radians; the loader converts).
fn foes(p: *Emit) void {
    // THE KNOT — homes sit >=12 m off the x~0 avenue so a straight run down the path doesn't
    // wake them, and so the hero-gait --shot stays clean: they guard the flanks and the
    // graveyard. Seeds and scales vary so the knot never moves as one body.
    ...
    // Faces back up the avenue (+Z) so the hero meets its eye as they close in.
...
fn fallenCity(p: *Emit) void {
    // The processional way carries on north. In env.zig this was a while-loop stepping 11 m
    // and picking pillar-or-broken per side; that IS a `line` — segments nose to tail from a
    // kind mix — so it becomes two of them, one per side, and the spacing is now a dial.
    ...
    // THE CHAPEL — roofed over its altar end, doorway facing the processional way (yaw 270
    // opens EAST). Three torches, not four: with a 6 m radius each, four filled the nave
    // evenly and there was nothing left to light.
    const cx: f32 = -30.0;
    const cz: f32 = -66.0;
...
fn theTarn(p: *Emit) void {
    ...
    // Drowned ruin standing IN the sheet, reed beds straddling the shoreline, willows leaning
    // over the margin. All three were angle+radius loops in env.zig; all three are discs.
    ...
    // LILY PADS float ON the water, so they are the one scatter that must NOT avoid it — and
    // they come in RAFTS from a rootstock, not sown evenly, so each raft is its own disc with
    // the area-uniform bias that packs it toward its centre.
    var raft: i32 = 0;
...
fn oldWood(p: *Emit) void {
    // THE CANOPY. Density rises toward the west edge, so walking in feels like entering it.
    // env.zig wrote this as a bespoke loop because the Placer had no mix and no gradient; it
    // is one belt now, and both are dials. Species MIXED — great trees for mass, conifers for
    // the skyline, birches for a colour you can pick out at distance, snags so not everything
    // is alive. Weights by repetition reproduce the old roll thresholds.
    const CANOPY = [_]Kind{
    ...
    // The canopy does its OWN shaping, so it opts out of the cover field: gating on both
    // thins the wood twice, and the second thinning is invisible in the numbers.
    canopy.field = false;

    // The understorey: what turns a stand of trees into a WOOD — without it you see straight
    // through to the far edge.
...
// The SPARSEST region on purpose — it is what makes the wood and the city feel dense — but
// sparse means "few tall things", not "empty ground": the heath cover underfoot is thick.
...
// ── THE EDGE ───────────────────────────────────────────────────────────────────────────
// One op now: four walls of overlapping cliff segments plus the talus spilling off their
// feet. The step (6.5 against a 10.4 m segment) is a DEEP overlap on purpose — that is what
// turns a row of separate rocks into one continuous escarpment.
...
// The zone rects reproduce env.zig's `region(x, z)` half-plane chain, in the same order — it
// tested x first, then z, and fell through to the home plain. First match wins, so the order
// of these five lines IS that chain, and the last one is the fallback.
//
// Each list keeps at least one LOW ground-hugger (clover / moss / heather): without something
// filling between the standing plants you get an even spacing of individuals with bare soil
// showing through, which reads as sparse no matter how many you place.
fn groundCover(p: *Emit) void {
    ...
    // NB no ivy in the city's cover even though it belongs to the city: ivy is a CLIMBER, and
    // dropped on open ground its runners stand up unsupported like beanstalks. It goes only
    // where there is stone to take it — see the `ivy` ops.
