...
/// `location: <name> <x> <z> <x1> <z1> [wet=..] [fog=..] [spore=..] [ember=..] [soup=..] [blend=..]` — the rectangle is positional and the sky is optional, because most locations will never have anything to do with the weather.
fn parseLocation(it: *std.mem.TokenIterator(u8, .any)) !Location {
    ...
    while (it.next()) |tok| {
        ...
        } else if (std.mem.eql(u8, key, "soup")) {
            l.soup = try band01(val);
...
test "a location round-trips through the format with and without its weather" {
    ...
    const text =
        ...
        "location: hollow 60 60 90 90 soup=0.600\n";
    try parse(text, m, &line);
    try std.testing.expectEqual(@as(usize, 4), m.nlocations);
