...
pub fn build(b: *std.Build) void {
    ...
    // No step may install or run these two — Step.Compile passes -fno-emit-bin only while nothing asks for the binary. 1.4 s against 8.4 s.
...
/// Assets added to the exe but not to `check` make `check.cmd` pass on a build that cannot link.
...
/// A module missing from main.zig's `test {}` block reports "all tests passed" with its tests unrun. Search the block, not the file — a top-level import pulls no tests in.
