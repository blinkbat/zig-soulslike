...
// THE SHARED SCAFFOLD, ALIASED NOT TRANSCRIBED — he walks on `hero.advanceGait` + `legChain`, which read 5..10.
...
const CLUB = heromod.HELD;
// His own six, past the hero's last.
const JAW = heromod.N + 0;
const TOEL = heromod.N + 1;
const TOER = heromod.N + 2;
const HUMP = heromod.N + 3;
const CLAVL = heromod.N + 4;
const CLAVR = heromod.N + 5;
const N = CLAVR + 1;
...
pub const Ogre = struct {
    ...
    fn parryable(self: *const Ogre) ?f32 {
        const left = foe.parryOpen(self, self.toImpact()) orelse return null;
