...
pub fn group(k: Kind) Group {
    return switch (k) {
        ...
        .giltarch, .muqarnas, .giltdome, .minaret, .jali, .giltcolumn, .giltbasin, .giltfinial, .giltleaf, .marblefloor, .marbleslab, .marblestair => .gilt,
        .ashcrag, .stalagmite, .menhir, .stonecarve => .ash,
        .merchanthut, .packstack, .trestletable, .goodsrack, .awning, .rugpile, .scalepost, .waterjars, .hitchrail => .market,
        .emberrock, .emberrocks, .burningrock, .cindercone, .firespire, .emberpillar, .emberarch, .basaltcolumns, .crackedslab, .magmavein, .lavacrust, .emberbed, .scoria => .firelands,
        .sunspire, .palacegate, .palacehall, .emeraldvault, .palacestair, .palacewall, .palacecolumn, .fallencolumn, .palaceterrace, .watcheridol, .sunobelisk, .starbasin, .palacebanner, .waystele, .emeraldshard, .palacekiosk => .palace,
        .saguaro, .saguaro2, .deadsaguaro, .joshua, .ocotillo, .agave, .yucca, .cholla, .pricklypear, .barrelcactus, .deadbrush, .tumbleweed, .sanddune, .sandripples => .desert,
    };
}
...
pub fn solidMat(m: f32) bool {
    ...
    if (id >= @typeInfo(gfx.Mat).@"enum".fields.len) return false;
    return switch (@as(gfx.Mat, @enumFromInt(id))) {
        .plain, .stone, .wood, .steel, .marble, .bark, .gilt, .rock => true,
        .cloth, .leather, .skin, .hide, .plant, .water, .flame, .smoke, .ember, .fog, .waterfall => false,
    };
}
...
test "A CLIFF'S COLLIDERS HUG ITS OWN ROCK — fitted off the masses, measured against the mesh" {
    ...
    for (rock.CLIFF_PROPS, 0..) |row, i| {
        var b = rock.cliffBuildOpt(row.seed, row.kind, null);
        defer b.deinit();
