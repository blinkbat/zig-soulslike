...
pub fn deadLimbTinted(b: *Builder, rng: *mathx.Rng, root: rl.Vector3, a: f32, reach: f32, rise: f32, r0: f32, twigs: i32, bark: rl.Color, heart: rl.Color) void {
    ...
    const drop = rise * rng.range(-0.7, 0.05);
    ...
    b.addBlob(tip, v3(r2 * 1.7, r2 * 1.3, r2 * 1.7), 3, 5, heart);
    var t: i32 = 0;
    while (t < twigs) : (t += 1) {
...
pub fn treeMesh(shader: rl.Shader) rl.Model {
    ...
    b.setMat(.bark);
    b.addBlob(v3(bend.x * 0.7 + 0.20, 1.25, bend.z * 0.7 - 0.06), v3(0.09, 0.14, 0.09), 3, 6, IRON);
    var br: i32 = 0;
    while (br < 6) : (br += 1) {
...
pub fn bigTree2(shader: rl.Shader) rl.Model {
    return bigTreeMesh(shader, .{ .seed = 7011, .trunk = 3.4, .spread = 1.22, .lift = 0.30, .gold = 0.42 });
}
pub fn bigTree3(shader: rl.Shader) rl.Model {
    return bigTreeMesh(shader, .{ .seed = 7023, .trunk = 5.6, .spread = 0.82, .lift = 0.85, .gold = 0.22 });
}
...
pub fn willowMesh(shader: rl.Shader) rl.Model {
    ...
    b.setMat(.plant);
    b.addBlob(crown, v3(1.9, 1.15, 1.85), 5, 8, LEAF_DK);
    var fh: i32 = 0;
    while (fh < 8) : (fh += 1) {
        const a = rng.angle();
...
pub fn coniferMesh(shader: rl.Shader) rl.Model {
    ...
    b.addBlob(v3(0, H * 0.99, 0), v3(0.34, 0.65, 0.34), 3, 6, NEEDLE);
    var g: i32 = 0;
    while (g < 3) : (g += 1) {
        const a = rng.angle();
...
pub fn rotLogMesh(shader: rl.Shader) rl.Model {
    ...
    var i: i32 = 0;
    while (i < N) : (i += 1) {
        const t0 = @as(f32, @floatFromInt(i)) / N;
    ...
    while (j < 9) : (j += 1) {
        ...
        const x = (t - 0.5) * ROTLOG_L * 0.86;
        const y = 0.42 - sag * mathx.sinf(t * std.math.pi);
        const w = 0.13 * mathx.sinf(t * std.math.pi) + 0.03;
    ...
    while (k < 5) : (k += 1) {
        ...
        const x = (t - 0.5) * ROTLOG_L;
        const sd: f32 = if (rng.float() < 0.5) -1.0 else 1.0;
        const r = rng.range(0.10, 0.20);
        b.addBlob(v3(x, 0.42 - sag * mathx.sinf(t * std.math.pi) + rng.range(-0.05, 0.14), sd * 0.34), v3(r, r * 0.24, r * 0.7), 2, 6, if (rng.float() < 0.5) CAP_PALE else CAP_BROWN);
    }
    b.addBlob(v3(rng.signed() * 0.6, 0.66, rng.signed() * 0.10), v3(1.0, 0.06, 0.30), 3, 7, MOSS_DK);
    tuftInto(&b, &rng, rng.signed() * 1.6, 0.55, 0.55);
    return b.toModel(shader);
}
...
pub fn deadfallMesh(shader: rl.Shader) rl.Model {
    ...
    b.setMat(.bark);
    const hub = v3(rng.signed() * 0.2, DEADFALL_H * 0.62, rng.signed() * 0.2);
    var i: i32 = 0;
    while (i < 7) : (i += 1) {
        const a = rng.angle();
        ...
        const r = rng.range(0.09, 0.17);
        b.addCapsule(foot, head, r, r * 0.7, 6, art.weathered(BARK_DK, BARK_OLD, @as(f32, @floatFromInt(i)) / 6.0));
        b.addDome(head, dir, r * 0.68, 5, art.PUNK_DK);
        if (rng.float() < 0.55) {
            const mid = mathx.lerpV(foot, hub, rng.range(0.3, 0.7));
    ...
    while (j < 8) : (j += 1) {
        const a = rng.angle();
        const d = rng.range(0.4, 1.6);
        const r = rng.range(0.04, 0.10);
        b.addCapsule(
            v3(mathx.cosf(a) * d, r, mathx.sinf(a) * d),
            v3(mathx.cosf(a) * d + rng.signed() * 0.5, r * 0.9, mathx.sinf(a) * d + rng.signed() * 0.5),
            r,
            r * 0.6,
            5,
            BARK_OLD,
    ...
    b.setMat(.plant);
    b.addBlob(v3(hub.x, 0.10, hub.z), v3(1.1, 0.07, 1.0), 3, 7, MOSS_DK);
